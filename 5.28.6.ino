...
bool sentRemainingTime = false; // 초록불 상태에서 Remaining Time 메시지를 보냈는지 여부
  ...
  if (digitalRead(switchPin) == LOW) { // 신호등 스위치가 눌렸을 때
    if (!switchPressed) {
      ...
      if (switchPressCount % 2 == 1) { // 홀수 횟수로 눌림
        ...
        if (!isBluetoothConnected) { // 처음 연결되었을 때
        ...
        if (isLedOn) {
          ...
          if (!sentRemainingTime && remainingTime > (greenLightDuration / 1000 - 7)) {
            String message = "Remaining Time: " + String(remainingTime);
            bluetooth1.write(message);
            sentRemainingTime = true;
          }
