...
SoftwareSerial Arduino_to_Radar_Serial(RX, TX);   // 레이더 소프트웨어 시리얼 객체명 선언(설정 보드의 RX 핀, 설정보드의 TX 핀)
SoftwareSerial BTSerial(10, 11);                  // 블루투스 시리얼 객체명 선언 & 핀 설정
...
void loop()
{
  ...
  //34000*초음파가 물체로부터 반사되어 돌아오는시간 /1000000 / 2(왕복값이아니라 편도값이기때문에 나누기2를 해줍니다.)
